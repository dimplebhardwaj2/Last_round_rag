// Professional line icons (Lucide path data, MIT) as inline SVG — no CDN, works offline.
//   icon("mic")              -> svg string (for JS-built DOM)
//   hydrate(root)            -> fills every <span data-icon="mic"></span> in static HTML
...
export function hydrate(root = document) {
  root.querySelectorAll("[data-icon]").forEach((n) => {
    n.innerHTML = icon(n.dataset.icon, { size: +(n.dataset.size || 22) });
  });
}
